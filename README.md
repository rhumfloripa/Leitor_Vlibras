...
Como a aplicação depende de scripts externos (Vimeo SDK e VLibras), ela precisa ser
servida por um servidor HTTP local (não funciona abrindo o arquivo direto via `file://`).

Escolha um dos servidores inclusos (todos abrem o navegador automaticamente em
`http://localhost:8000/index.html`):

| Runtime | Como iniciar |
|---------|--------------|
| **Node.js** (recomendado) | duplo-clique em `iniciar_servidor_node.bat` — ou `node servidor.js` |
| **PowerShell** (já vem no Windows) | duplo-clique em `iniciar_servidor_powershell.bat` |
| **Python 3** | duplo-clique em `iniciar_servidor.bat` — ou `python servidor.py` |
...
1. **`window.plugin.translate(texto)`** — o widget oficial do VLibras
   ...
   `translate()` delega para o `Player.translate()` interno. Isso permite tradução
...
2. **Sincronização por `gloss:end`** — o Player emite o evento `gloss:end` quando o
   avatar termina de sinalizar uma frase. A fila escuta esse evento para só então
   enviar a próxima legenda, evitando cortes.
...
iniciar_servidor_node.bat         # launcher (Node)
iniciar_servidor_powershell.bat   # launcher (PowerShell)
iniciar_servidor.bat              # launcher (Python)
```
